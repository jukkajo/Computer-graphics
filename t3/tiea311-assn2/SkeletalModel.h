...
public:
    ...
    //tämä on varmaan hyvä paikka tälle
    void rekursio(Joint* solmu);



    // 1.1. Implement this method with a recursive helper to draw a sphere at each joint.
    void drawJoints();
    void rekursio2(Joint* solmu);
    ...
    void wToJoint_rekursio(Joint* solmu);
    // 2.3.2. Implement this method to compute a per-joint transform from
    // joint space to world space in the CURRENT POSE.
    void updateCurrentJointToWorldTransforms();
    void jToWorld_rekursio(Joint* solmu);
